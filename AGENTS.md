...
- `RUN_3_REPORT.md` - Comprehensive post-mortem engineering report analyzing root causes of failure in Run #3 (located in `logs/`).
- `RUN_4_REPORT.md` - Comprehensive success report detailing architecture leaps, test set metrics, and benchmark results in Run #4 (located in `logs/`).
- `RUN_5_PREPARATION_REPORT.md` - Technical engineering report detailing Run #5 readiness, combinatorial template bank, root data integrity, and DDP training matrix (located in `logs/`).
...
- `tests/` - Directory housing all unittest suites.
  ...
  - `test_fix_root_data.py` - Tests for root data auditing, stimulus citations, and section cleanup.
  ...
  - `test_template_bank.py` - Tests for Combinatorial Template Bank, layout formatting styles, and sampling helpers.
...
- `scratch/` - Directory housing temporary and utility debug scripts.
  - `test_toeic_eval.py` - Evaluation script for testing TOEIC exam sequence labeling formats on mmBERT/ModernBERT models.
  - `test_toeic_deep.py` - Deep evaluation script covering bilingual, invoice, and blank variants for TOEIC.
  - `test_toeic_remaining.py` - Evaluation script testing chat chains, triple passages, sentence insertions, and OCR artifacts.
  - `test_customer_formats.py` - Evaluation script covering IELTS, SAT, ĐGNL/ĐGTD, JLPT, and OCR metadata formats.
  - `generate_bio_lit_50.py` - Batch generation script for 25 Literature and 25 Biology exams using Codex.
  - `generate_toeic_lit_50.py` - Batch generation script for 25 Literature (9 formats) and 25 TOEIC (Parts 1-7) exams using Codex.
...
- `output/` - Output directory containing generated exams, curricula, and datasets (gitignored).
  - `dataset/` - Prepared tokenized sequence labeling dataset splits (`train.jsonl`, `val.jsonl`, `test.jsonl`, `raw_exams.jsonl`, `label_mapping.json`, and `xml/`).
  - `exams/` - Synthetic mock exams generated by DeepSeek / LLM providers (`exam_*.json`).
  - `real_annotated/` - Audited real OCR exam structures in JSON/XML formats (`merged.json`, `merged.xml`).
...
- `src/` - Main source package directory.
  ...
  - `data/` - Data Preparation & Real OCR Pipeline.
    - `fix_root_data.py` - Audits and permanently repairs root XML and JSON data files directly on disk.
  ...
  - `generation/` - Core Data Generation Subpackage.
    ...
    - `template_bank.py` - Combinatorial template bank containing 168+ authentic section headers, explanation prefixes, barem rubrics, and answer key grids.
...
- To audit and repair root data sources: `pixi run fix-root-data`
- To consolidate raw synthetic & real exams: `pixi run consolidate-raw`
- To prepare the offline tokenized dataset: `pixi run prepare-offline-dataset`
...
- **Subjects:** `economics_law`, `geography`, `history`, `math_algebra`, `math_geometry`, `physics`, `chemistry`, `biology`, `english`, `literature`, `toeic`.
...
- `section`: Labels structural exam section divisions, part headers, and group instructions (e.g. "PHẦN I. Câu trắc nghiệm...", "PHẦN II. Đúng sai", "Part 1: Photographs", "Chủ đề Địa lí: ...", "Chuyên đề 1: ..."). Administrative headers, footers ("HẾT"), and question prefixes are untagged or assigned their proper entity tags.
- `explanation`: Labels solutions, detailed explanations, marking scheme tables (barem), and answer key grids.
